...
bool DirectX11Framework::Init(void* aWindowHandle)
{
	...
	size_t width;
	size_t height;
	if (GetWindowRect(static_cast<HWND>(aWindowHandle), &rect))
	{
		width = rect.right - rect.left;
		height = rect.bottom - rect.top;
		AddMemoryUsage(static_cast<size_t>(DirectX11Framework::FormatToSizeLookup[DXGI_FORMAT_R8G8B8A8_UNORM] * width * height), "Backbuffer", "Engine");
	}
