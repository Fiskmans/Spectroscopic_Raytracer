...
bool Camera::Init(float aFoV, CU::Vector2<float> aResolution, float aNear, float aFar)
{
	myFarPlane = aFar;
	myNearPlane = aNear;
	myFOV = aFoV;
...
V3F Camera::Unproject(V3F aProjectedPoint) const
{
	auto view = GetTransform();
	auto invViev = CommonUtilities::Matrix4x4<float>::GetRealInverse(view);

	auto invProjectionMatrix = CommonUtilities::Matrix4x4<float>::GetRealInverse(CommonUtilities::Matrix4x4<float>::GetRealInverse(view) * GetProjection(false));
	V4F result =
	{
		(aProjectedPoint.x - .5f) * 2.f,
		(aProjectedPoint.y - .5f) * -2.f,
		aProjectedPoint.z,
		1
	};
