...
static ImVector<ID3D11ShaderResourceView*>      g_Textures;
...
static void ImGui_ReleaseTexture(ID3D11ShaderResourceView* texture);
...
void ImGui_DestroyTexture(ImTextureID texture)
{
    ...
    ID3D11ShaderResourceView* texture_object = (ID3D11ShaderResourceView*)(texture);

    ImGui_ReleaseTexture(texture_object);

    for (ID3D11ShaderResourceView** it = g_Textures.begin(), **itEnd = g_Textures.end(); it != itEnd; ++it)
    {
...
{
    if (ID3D11ShaderResourceView* tex = (ID3D11ShaderResourceView*)(texture))
    {
        ID3D11Resource* res;
        tex->GetResource(&res);
        ID3D11Texture2D* res2d;

        if (res->QueryInterface(&res2d) == S_OK)
        {
            D3D11_TEXTURE2D_DESC desc2d;
            res2d->GetDesc(&desc2d);
            return desc2d.Width;
        }
    }
