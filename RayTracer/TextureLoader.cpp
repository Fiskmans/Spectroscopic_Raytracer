...
Texture* GeneratePerlin(ID3D11Device* aDevice, CommonUtilities::Vector2<size_t> aSize, V2F aScale, unsigned int aSeed, D3D11_RESOURCE_MISC_FLAG aFlag)
{
	...
	for (size_t y = 0; y < aSize.y; y++)
	{
		...
		{
			color[y * aSize.x + x].x = static_cast<float>(generators[0].noise(x / static_cast<double>(aScale.x) / aSize.x * 255.0, y / static_cast<double>(aScale.y) / aSize.y * 255.0, 0.0));
			color[y * aSize.x + x].y = static_cast<float>(generators[1].noise(x / static_cast<double>(aScale.x) / aSize.x * 255.0, y / static_cast<double>(aScale.y) / aSize.y * 255.0, 1.0));
			color[y * aSize.x + x].z = static_cast<float>(generators[2].noise(x / static_cast<double>(aScale.x) / aSize.x * 255.0, y / static_cast<double>(aScale.y) / aSize.y * 255.0, 2.0));
	...
	WIPE(data);
	data.pSysMem = color;
	data.SysMemPitch = static_cast<UINT>(sizeof(CommonUtilities::Vector4<float>) * aSize.x);
	data.SysMemSlicePitch = static_cast<UINT>(sizeof(CommonUtilities::Vector4<float>) * aSize.x * aSize.y);
